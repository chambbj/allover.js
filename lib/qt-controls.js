THREE.QTControls = function ( camera, domElement ) {
  ...
  this.mousemove = function( event ) {
    if ( this.mouseStatus > 0 ) {
      ...
      if ( this.mouseStatus === 1 ) {
        this.destinationRotation.xy = this.rotationOnMouseDown.xy + dx * -0.02;
        this.destinationRotation.z = this.rotationOnMouseDown.z + dy * 0.02;
      } else if ( this.mouseStatus === 3 ) {

        var downvector = -Math.cos(this.rotation.z);
        var sidevector = Math.sin(this.rotation.xy);

        this.center.x += this.panSpeed * (Math.sin(this.rotation.xy) * dx + Math.cos(this.rotation.xy) * dy * sidevector);
        this.center.y += this.panSpeed * (-Math.cos(this.rotation.xy) * dx + Math.sin(this.rotation.xy) * dy * sidevector);
        this.center.z -= this.panSpeed * dy * downvector;
  ...
  this.update = function( delta ) {
    ...
    this.debug.html("X position: " + Math.round(this.camera.position.x) +
      ...
      " &mdash; Y rotation: " + Number((this.rotation.z).toFixed(3)));
  ...
  this.reset = function() {
    this.adjustCameraPosition(this.INITIAL_ROTATION);
  };
  ...
  this.adjustRadius = function(delta) {
    this.destinationRadius -= this.zoomSpeed * delta;
