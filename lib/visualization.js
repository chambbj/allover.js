...
(function() {
    ...
    this.init = function(url, container, options) {
        ...
        this.resetViewButton = $("<a>Reset view</a>");
    ...
    this.animate = function() {
        ...
        (function animloop(){
            requestAnimFrame($.proxy(animloop, this));
            this.render();
        }).apply(this);
    };
    ...
    this.pointloader = function(headers, rawpoints, data, rawpointcounter, pointBucket) {
        ...
        if (this.selectedColors === null) {
            if (this.colorizedPoints) {
                ...
                imageryButton.addClass('disabled');
        ...
        this.camera.position.y = midy;
        this.camera.position.z = maxz + 200;
        var startingPosition = this.camera.position.clone();
        var startingQuaternion = this.camera.quaternion.clone();

        this.resetViewButton.on('click', function() {
            that.camera.position.copy(startingPosition);
            that.camera.quaternion.copy(startingQuaternion);
        });
